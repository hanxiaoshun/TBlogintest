...
function get_xcode() {
    $.post("/get_xcode", {},
        function(data) {
            ...
            // alert(data.status);
            if (data.status === "ok") {
                alert("二维码获取成功,请扫描右侧二维码,如扫描失败,请重新获取二维码");
                $("#id_td_img_xcode").html("<img id='id_img_xcode' src='/static/img/xcode.png'>");
                alert("扫描二维码之后请等待3秒以后点击 '发送验证码' ")
            } else {
                $("#id_td_img_xcode").html("");
                alert("二维码获取异常,请重新获取");
            }
        });
}

function finish_xcode() {
    $.post("/finish_xcode", {},
        function(data) {
            if (data.status === "ok" && data.need === "yes") {
                $("#p_code").show();
                $("#p_code_enter").show();
            } else {
                $("#p_code_enter").hide();
...
function phone_captcha_Code() {
    let p_code = $("#p_code").val();
    if (p_code.length === 6) {
        $.post("/phone_captcha_Code", { "phone_code": p_code },
            function(data) {
                if (data.status === "ok") {
                    $("#p_code").hide();
                    $("#id_td_img_xcode").hide();
                    alert("登录成功");
                } else {
                    alert("手机验证码不正确,请重新尝试");
                }
            });
    } else {
        alert("请您输入6位数手机验证码！");
